...
import 'package:lifelink/feature/profile/data/datasources/local/profile_local_datasource.dart';
import 'package:lifelink/feature/profile/data/datasources/profile_datasource.dart';
import 'package:lifelink/feature/profile/data/datasources/remote/profile_remote_datasource.dart';
...
class ProfileRepository implements IProfileRepository {
	...
	Future<Either<Failure, Map<String, dynamic>>> updateProfile(
		...
		try {
			final updatedData =
					await _remote.updateProfile(token: token, data: data);

			final cached = await _local.getCachedProfile(userId);
			if (cached != null) {
				final updatedCache = ProfileHiveModel(
					userId: cached.userId,
					firstName: cached.firstName,
					lastName: cached.lastName,
					email: cached.email,
					imageUrl: cached.imageUrl,
					bloodGroup: data['bloodGroup'] ?? cached.bloodGroup,
					phoneNumber: data['phoneNumber'] ?? cached.phoneNumber,
					emergencyContact: data['emergencyContact'] ?? cached.emergencyContact,
				);
				await _local.cacheProfile(updatedCache);
			}
